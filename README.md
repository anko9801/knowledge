...
- [計算機](https://anko9801.github.io/knowledge/cs) — 計算量理論・型と計算・プログラムの構成・情報理論・並行と分散・計算機の構成・暗号理論。構成だけ決めてあり、本文はこれから
...
制約は二種類ある。**機械の側**（速い記憶は高い、依存のある命令は重ねられない）が
「計算機の構成」を決め、**人の側**（同時に頭へ置ける量は決まっている）が
「プログラムの構成」を決める。後者は実験結果であって定理ではないので、
概念グラフでは `empirical` の辺でだけ繋いである。既定の経路探索には出てこない。
「作業記憶は 4±1」から「goto をやめろ」は導けないからで、そこは動機にとどまる。
導けるのはその先——進行状況を有限の座標で書けるか（Hoare 論理）、実装を替えても
外から区別できないか（表現独立性）、式を値で置き換えてよいか（Church--Rosser）。
