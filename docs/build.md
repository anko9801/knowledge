...
`figure` は**既定で中央寄せ**になる。`show` rule で中身をブロックに差し替えても
その寄せは効いたままで、**中の段落まで巻き込む**。主張の枠は figure なので、
紙面では定義や例の本文が全部中央に寄っていた。
画面側は CSS が寄せを持っているので無事で、**PDF を開くまで分からなかった。**

`set align(left)` を主張のブロックに入れて直してある。図の説明も同じ理由で
中央寄せだったので、`template.typ` の paged 側で `show figure.caption` を当てている。

出力が二つある以上、**片方を見て直したときはもう片方も見る。**
PDF は `pdftoppm -png -r 70 -f 1 -l 1 dist/…/2.pdf out` で 1 枚だけ出せる。
...
### 本文の語に、概念のひとことを添える（`#term`）

同じ仕掛けを、主張ではなく**用語**に当てる。判断は `docs/reader.md`
「本文の語からも引けるようにする」で、ここは書き方だけ。

```typst
#term[多様体]                            名前が `concepts.ts` と同じなら、これだけ
#term("sigma-algebra")[$sigma$ 加法族]     語形が違う・数式が入るときは id を渡す
```

引き先は `src/data/concepts.ts` の `label` と `id`。組み立ては `src/lib/term.ts`
（純関数、テストあり）で、ローダーが `attachPeeks` の**あと**に呼ぶ。
順番を逆にすると、複製された主張の中の用語が取っ手を共有する。

**大半は自動で付く**（`linkTerms`）。`#term` を書くのは、自動が拾わないところ
——2 字以下の名前、多義語（`AMBIGUOUS`）、語形が違うとき——だけでよい。
手で置いたぶんが先に印になり、自動はそこを触らない。

取っ手の接頭辞は `tm`。`peek.ts` の `pk`、`page-peek.ts` の `pp` と分けてある。
同じページに三種類とも出るので、揃えると触る端末で別のものが開く。

**引けなければビルドを止める。** 素通しにすると、概念の名前を変えたときに
印だけが本文に残り、何も出ないページが配られる。

紙面には索引が無いので、`target()` を見て**点線の下線だけ**にしてある。

#### 踏んだ穴

**`content.config.ts` の `DEPENDS_ON` に足すのを忘れた。** `term.ts` を直しても
digest が変わらないので、**古い HTML が出続ける**。あのファイルのコメントが
warning していたとおりの壊れ方で、実際に一度踏んだ。`concepts.ts` も入れてある
（`gist` を直したら組み直したい）。

**base が二重スラッシュになった。** `rewriteInternalLinks` は base を
`/knowledge/`（末尾スラッシュ付き）で受ける前提で書いてあるので、
そのまま繋ぐと `/knowledge//concepts/…` が出る。`term.ts` 側で末尾を吸っていて、
`/knowledge/` `/knowledge` `/` の三通りをテストで固定してある。
...
**`scale` も中身ごと落ちる。**pandoc は LaTeX の `\left[` を拡大した括弧として写すので、
外れると**和や積のかかる範囲が変わって見える**。実測 101 か所。括弧の大きさは
ブラウザが決めるので、そもそも Typst 側に要らない。

**`overline` は線だけが落ちる。**中身が残るぶん気づきにくいが、
**反ニュートリノがニュートリノに、共役 $\bar z$ が $z$ になる**。式の意味が変わる。
実測 20 か所。`accent(x, macron)` なら `<mover accent="true">` が出る。
