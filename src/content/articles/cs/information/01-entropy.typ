...
満たしてほしい性質を三つ書き下すと、#strong[それを満たす関数は一つしか無い]。
$-sum p log p$ は、誰かが思いついた式ではない。

だから天下りに式を出さず、#strong[何を満たしてほしいか]から書く。
...
#check[
  @def:axioms の三条件のうち、$log$ を呼び出しているのは #strong[3（分解できる）]である。
  なぜ「二段階に分けても合計が変わらない」から対数が出てくるのか。
][
  分解の条件が、#strong[積を和に変える関数]を要求するからである。

  二段階に分けるとき、場合の数は掛け算で増える（$m$ 通りのあと $n$ 通りなら $m n$ 通り）。
  いっぽう不確かさは足し算で合わさってほしい、というのが条件 3 である。

  $f(m n) = f(m) + f(n)$ を、連続（条件 1）かつ単調（条件 2）という縛りのもとで
  満たす関数は、定数倍を除いて $log$ しかない。だから
  @thm:uniqueness で「選ぶ余地が無い」と言える。

  #strong[対数は情報の性質ではなく、「掛け算で増えるものを足し算で測りたい」という
  要求の答え]である。だから同じ形が、掛け算で増える量を測る場所には毎回出てくる。
]
