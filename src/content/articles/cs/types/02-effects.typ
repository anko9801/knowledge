...
この二つは同じもののはずである。$x$ は $upright("read")$ の値なのだから、
$x$ と書いたところに $upright("read")$ と書き直しても変わらない。

変わる。左は一行読んで二回使い、右は#strong[二行読む]。
...
$upright("read")$ は参照透過でない。
そして置き換えができないと、#link("/cs/types/1")[前回]の道具が全部使えなくなる。
簡約の順序を変えても行き着く先が変わらないから式を値で置き換えてよい、
というのが前回の出発点だった。
...
ここは信じにくいところだと思う。
状態と失敗を同時に使うプログラムなら、毎日書かれている。

書けている。書けているのは $T compose S$ が一般に作れるからではなく、
#strong[その組み合わせについて入れ替える手立てを、誰かが一度書いたから]である。
