...
#check[
  偏微分が全部あれば、@prop:partial の形にヤコビ行列を書き下せる。
  行列が書けるなら線形写像も決まる。近似する相手はもう手元にある。
  #strong[それなのに微分可能でないことがあるのはなぜか。]
][
  #strong[写像を書けることと、それが近似になっていることが別だからである。]

  @def:derivative が要求しているのは、$bold(h)$ が
  #strong[どの向きから $bold(0)$ へ近づいても]誤差が $abs(bold(h))$ より速く消えること。
  向きの取り方に条件が付いていない。

  偏微分が見ているのは、$bold(h) = t bold(e)_j$ という軸方向だけである。
  $n$ 本の直線の上での振る舞いから、平面全体での振る舞いは出てこない。

  上の例がその差になっている。軸に沿えば恒等的に $0$ なので偏微分は両方 $0$、
  ヤコビ行列は零行列になる。ところが $y = x$ に沿うと値が $1\/2$ のままで、
  零写像が近似になっていない。連続ですらない。

  だから @prop:partial は片道である。微分可能なら成分が取れるが、
  #strong[成分が揃っても、もとの写像があるとは限らない]。
]
...
= 近似できることと、近似が役に立つことは違う

微分は最良の線形近似だと書いた。#strong[「最良」は、他の線形写像より誤差が速く
小さくなる、という意味しかない。] どれだけ小さいかは何も言っていない。

近似が使える範囲は、微分の側からは出てこない。$e^(-1\/x^2)$ を $0$ の近くで
考えると、微分は全部 $0$ になる。線形近似も、二次も、何次まで取っても $0$ である。
それでも関数は $0$ ではない。#strong[Taylor 展開が全部消えるのに、関数が残る。]

こういう関数があるせいで、「微分が全部分かれば関数が分かる」は成り立たない。
どこまで成り立つのかは第 3 回の主題になる。そして成り立たない側が、
実は多様体を作るのに要る道具になる。

もう一つ。@def:derivative では $lim$ と $abs(dot)$ を使った。長さを測っている。
#strong[だが極限を言うのに長さは要らない。] 開集合だけで足りる。
その分離が、多様体を定義するときに効いてくる。次回はそこを見る。
