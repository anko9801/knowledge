...
数直線の全体と、開区間 $(-1,1)$ を並べる。
$tan$ で引き伸ばせば片方からもう片方へ移り、戻すこともできる。
切れ目を作らず、一対一で、両向きに。#strong[形としては同じものである。]

ところが $(-1,1)$ の中で $0.9$、$0.99$、$0.999$、と並べていくと、
間隔はいくらでも詰まっていくのに、行き先が中に無い。
数直線のほうでは、こう並べれば必ず行き先がある。

#strong[同じ形なのに、片方にだけ穴が空いている。]

答えは、形が全部を決めていないところにある。
伸び縮みで移り合っても、#strong[どれだけ離れているか]は移らない。
上の列が「詰まっていく」と言えたのは長さを測ったからで、
長さは形の側の言葉ではない。

#table(
  columns: (auto, 1fr),
  [形だけで決まる], [連続、コンパクト、連結、収束],
  [長さが要る], [長さそのもの、一様連続、Cauchy 列、#strong[完備性]],
)

だから完備性は位相の性質ではない。
それでも捨てられない。#strong[「極限が存在する」と言えるかどうか]が
そこに懸かっているからである。

#check[
  $RR$ と $(-1, 1)$ は同相である。位相の言葉で言えることは全部同じはずなのに、
  片方だけが完備になる。#strong[同相写像が保たないものは何か。]
][
  #strong[どの列が Cauchy 列か]である。

  収束するかどうかは位相だけで決まるので、同相写像で保たれる。だが Cauchy 列の
  定義には「点どうしの距離が縮む」が入っていて、これは距離が要る。
  同相写像は距離を保たないので、Cauchy 列を Cauchy 列に写すとは限らない。

  実際 $(-1,1)$ で端に寄っていく列は Cauchy だが、$RR$ へ写すと無限へ発散して
  Cauchy でなくなる。#strong[「収束しない Cauchy 列がある」という言い方自体が、
  距離を固定して初めて意味を持つ。]完備性が位相の性質でないのは、このためである。
]
