...
= よく振る舞う関数ほど、貼り合わせに使えない

$C^infinity$ を選んだ理由は、山形関数が作れることだった。ある範囲では $1$ で、
外では厳密に $0$ で、途中が滑らかに繋がっている関数である。

その一つ上の階層では、これが作れない。$C^omega$ は解析的、つまり各点で
Taylor 級数が収束して関数と一致する。@thm:identity が言う通り、
そういう関数は一点の近傍で決まれば全体が決まってしまう。
#strong[どこかで $0$ なら、全部 $0$ である。]

つまり階層を上がるほど良くなる、ではない。#strong[上げすぎると、局所で何かを
するという操作ができなくなる。] 多様体を地図の貼り合わせで作るのに、
局所で切り貼りできない関数しか無い世界では、$1$ の分割が組めない。

#strong[$C^infinity$ は、良さの上限ではなく釣り合いの点である。] 微分は何回でも
できてほしい。しかし一点の情報が全体を縛るほど強くはあってほしくない。
その二つを同時に満たす階層が、たまたま一つあった。
