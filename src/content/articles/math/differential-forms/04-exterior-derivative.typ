...
#check[
  @thm:unique の一意性の証明は、座標を取って書き下すところから始まる。
  座標に依らないことを示したいのに、座標を使っている。
  #strong[これは循環していないか。]
][
  #strong[していない。示しているのは「どの座標で書いても同じ操作になる」ことだからである。]

  証明が座標でやっているのは、四条件から $dif omega$ の値が
  #strong[一通りに決まってしまう]ことの確認である。決まるのは値であって、
  決め方が座標に依るかどうかは別の話になる。

  そして値が四条件だけで決まるなら、別の座標で計算しても同じ値が出るしかない。
  条件のほうに座標が出てこないからである。

  #strong[座標は道具として使ってよく、結論に残らなければよい。]
  第 2 回で接ベクトルを定義したときも、同値関係の確認に座標を使って、
  どれを使うかには依らないことを別に見た。同じ形の手続きである。
]
...
そして四条件のどれにも計量が出てこない。
だから $dif$ は、長さも角度も定義されていない多様体の上でそのまま使える。
この連載が最後まで計量を入れずに進めるのは、この一点による。
...
この式が効く場面は多い。$cal(L)_X T = 0$ という形の条件は、
「$T$ が $X$ の流れで保たれる」——つまり $X$ が $T$ の対称性である——ことを表す。
$T$ に何を入れるかで、扱う対象が変わる。

#table(
  columns: (auto, 1fr),
  [$cal(L)_X g = 0$], [計量の対称性。$X$ は#link("/math/riemannian-geometry/1")[Killing ベクトル]],
  [$cal(L)_X omega = 0$], [$2$ 形式 $omega$ を保つ流れ。シンプレクティック幾何の出発点],
  [$cal(L)_X upright("vol") = 0$], [体積を保つ流れ。$RR^n$ なら $nabla dot X = 0$ に相当する],
)

@thm:cartan の実用上の値打ちは、
これらが#strong[すべて $dif$ と $iota_X$ の計算に落ちる]ことにある。
この連載では使わないが、名前だけ置いておく。
...
「閉だが完全でない形式がどれだけあるか」を数える理論が
#link("/math/differential-forms/7")[第 7 回]の主題になる。
そこで見るとおり、この個数は#strong[領域の位相だけで決まる]。
原点を抜いたことが効いていて、$RR^2$ 全体なら $omega$ は完全になる。
