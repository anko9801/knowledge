...
#check[
  置換積分ではヤコビアンを掛けろ、と習う。掛け忘れると答えが合わない。
  長らく#strong[計算のうっかり]だと思っていた。
  #strong[それがこの回の「関数は積分できない」と、どう繋がるのか。]
][
  #strong[掛け忘れではなく、運ぶものを取り違えている。]

  極座標の例で、$f equiv 1$ を関数として運ぶと $r$ が出てこない。
  出てこないのは忘れたからではなく、#strong[関数が座標の取り替えで何も変わらない]
  からである。値がそのままなので、掛けるべき係数を持ちようがない。

  $2$ 形式として運ぶと $dif x and dif y = r thin dif r and dif theta$ となり、
  $r$ が式のほうから出てくる。@prop:well-defined が言っているのは、
  この $r$ が重積分の変換則とちょうど釣り合う、ということである。

  だから「ヤコビアンを掛ける」は補正ではなく、#strong[形式として書けば最初から
  そこにあったもの]を、手で書き足しているだけだった。
]
