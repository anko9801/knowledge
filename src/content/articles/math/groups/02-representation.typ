...
群の元それぞれに $n times n$ 行列を割り当てたとする。
元ひとつにつき $n^2$ 個の数が並んでいる。

ここから#strong[対角和だけを残して、あとは全部捨てる]。$n^2$ 個が $1$ 個になる。
捨ててよい理由は一応ある。基底を取り替えると行列の中身は総取り替えになるが、
対角和は変わらない。中身のほうは、そもそも自分で選んだ基底の都合を含んでいた。

それにしても捨てすぎである。$n = 3$ なら $9$ 個が $1$ 個、
$n = 10$ なら $100$ 個が $1$ 個になる。

ところが、#strong[残った $1$ 個の数の並びから、元の割り当てが何だったかが
復元できる]。しかも復元の手続きは、二つの並びを掛けて足すだけで済む。
積分を一つも計算せずに「この積分は $0$ である」と言えるのは、これが理由である。

捨てた情報は、はじめから中身の側に無かった。以下、そこまでを追う。
...
#link("/math/linear-algebra/4")[線形代数 第 3 回]で見たとおり、
跡は縮約なので基底に依らない。さらに
...
表現が既約なものに分かれる、という出発点は、群の上で平均を取ることで示した。
どの元も同じ重みで足して、不変な内積を作る。

#strong[足せる群でしか、この議論は動かない。] 有限群なら個数で割ればよく、
コンパクト群なら全体の体積が有限なので割れる。#strong[非コンパクト群では割れない。]

Lorentz 群がその例である。ブーストの速さに上限が無いので、
全体を足すと発散する。実際、Lorentz 群の有限次元表現は
ユニタリにならず、既約分解の議論がそのまま持ち込めない。

ここは逃げ道が無いわけではなく、無限次元の表現を許すと話が繋がる。
ただし#strong[有限個の数値の表に落ちる]という、この回のいちばん強い性質は失われる。
指標の直交関係で分解が計算できたのは、表が有限だったからである。

#strong[対称性を仮定すると計算せずに分かることが増える]、というのはその通りだが、
増える量は群の形に依る。コンパクトなら大きく増え、そうでなければ別の道具が要る。
