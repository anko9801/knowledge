...
#show: post.with(
  ...
  order: 4,
  tags: ("線形代数",),
  summary: "テンソルのうち、物理に出てくるものはたいてい対称か反対称のどちらかである。応力は対称、電磁場は反対称。この二種類だけが特別扱いされる理由と、交代形式の次元が二項係数になることを見る。",
)

#layer("L0 ベクトル空間")[引数を入れ替えたときの振る舞いだけを見る。計量は不要]

= 物理のテンソルは、なぜどちらかに寄るのか

#link("/math/linear-algebra/3")[前回]並べた物理のテンソルを、もう一度眺めてみる。

#table(
  columns: (auto, 1fr),
  [対称], [計量 $g_(mu nu)$、応力 $sigma_(i j)$、慣性 $I_(i j)$、誘電率 $epsilon_(i j)$],
  [反対称], [電磁場 $F_(mu nu)$、角運動量 $L_(mu nu)$、面積要素、行列式],
)

$(0,2)$ 型テンソルの成分は $n^2$ 個あるのに、実際に出てくるものは
$T_(i j) = T_(j i)$ か $T_(i j) = -T_(j i)$ のどちらかを満たしている。
中途半端なものが、ほとんど出てこない。

偶然ではない。この記事はその理由と、
#link("/math/vector-analysis/1")[ベクトル解析 第 1 回]で数えた $1, 3, 3, 1$ の出どころを扱う。
...
物理量の分類として使える。中途半端なものが出てこないのは、
#strong[出てきてもすぐ二つに分けられて、それぞれが別の意味を持つ]からである。

#example[速度勾配の分解][
  流体の速度場の微分 $partial_j v_i$ は、対称でも反対称でもない。
  @prop:decomposition で分けると
  $ partial_j v_i = underbrace(frac(1, 2)(partial_j v_i + partial_i v_j), upright("ひずみ速度"))
    + underbrace(frac(1, 2)(partial_j v_i - partial_i v_j), upright("渦度")) $
  になる。対称部分は流体要素の変形を、反対称部分は回転を表す。
  同じことが弾性論でも起き、対称部分がひずみ、反対称部分が剛体回転になる。

  物理の側が「分けて別々の名前を付けている」わけである。
]
...
時空では同じ混同が起きない。電磁場が $6$ 成分の $F_(mu nu)$ として現れ、
ベクトルに見えないのは、これが理由である。
...
分類には引数を並べ替える群（対称群）の表現論が要り、
その道具が Young 図形である。

#example[Riemann 曲率テンソルは混合対称][
  一般相対論に出てくる $R_(mu nu rho sigma)$ は、
  最初の 2 本について反対称、後ろの 2 本についても反対称、
  かつ前半と後半を入れ替えると不変、という対称性を持つ。
  完全反対称でも完全対称でもない。

  独立成分の数が $4$ 次元で $20$ 個になるのは、この混合対称性を数えた結果である。
  「反対称なものは微分形式で書ける」という話が
  重力の主役に対しては素直に効かないのは、ここに理由がある。
]
...
対称テンソルにはこの二つが無い。
だから応力や計量は最後まで添字で書かれ、電磁場は $F = dif A$ と書ける。
