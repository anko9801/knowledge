...
#link("/math/linear-algebra/3")[前回]の材料は二つだけだった。$V$ と $V^*$ である。
...
#check[
  冒頭の流儀——変換則を満たすものをテンソルと呼ぶ——でも、計算はできる。
  物理の教科書はそれで何十年も通っている。
  #strong[それを定義として採らないのはなぜか。]
][
  #strong[座標を選ぶ前の対象が、一度も言われないからである。]

  変換則は#strong[成分の振る舞い]についての条件で、成分は基底を選んで初めて出てくる。
  つまりあの流儀は、選んだあとの見え方だけで対象を指そうとしている。
  何が変換しているのかは、書かれていない。

  実務でも困る。ある量がテンソルかどうかを確かめるのに、
  あらゆる座標変換を試すのか、という問いに答えられない。

  @def:tensor は先に対象を決める。多重線形写像であることは、
  基底を一つも選ばずに確かめられる。そして @thm:transform が
  #strong[変換則を定理として返す]。失っているものは無く、順序が変わっただけである。
]
...
これで#link("/math/linear-algebra/2")[第 1 回]の宿題が一つ片付く。
跡（トレース）が基底に依らないのは、
...
その事情は #link("/math/linear-algebra/7")[第 6 回]で清算する。
