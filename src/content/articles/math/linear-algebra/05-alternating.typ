...
#example[双線形形式の分解][
  ...
  #link("/math/linear-algebra/6")[次回]の行列式の話に直結する。
]
...
#check[
  冒頭では「入れ替えは対合、対合の固有値は $plus.minus 1$、
  だから二つに分かれる」と論じた。3 階でも「1 番と 2 番の入れ替え」は対合である。
  #strong[同じ議論がなぜ通らないのか。]
][
  #strong[通る。ただし分かれた先が、欲しかったものではない。]

  1 番と 2 番の入れ替えを $tau_(12)$ とすると、これは確かに対合なので、
  $(0,3)$ 型テンソルの空間は $tau_(12)$ の $plus.minus 1$ 固有空間に分かれる。
  分かれるのは#strong[「1,2 について対称」と「1,2 について反対称」]である。
  完全対称でも完全反対称でもない。

  完全対称・完全反対称であるとは、$tau_(12)$ と $tau_(23)$ の
  #strong[両方について同時に]固有ベクトルであることをいう。
  二つの対合の同時固有空間を取ることになる。

  ここが 2 階と違う。$S_2$ には入れ替えが一つしかないので、
  一つの対合の固有空間分解がそのまま答えになった。$S_3$ には三つあり、
  しかも $tau_(12) tau_(23) != tau_(23) tau_(12)$ で#strong[可換でない]。
  可換でない作用素は同時対角化できないので、
  同時固有空間だけでは全体を尽くさない。尽くさなかったぶんが混合対称である。

  #strong[「対合だから分かれる」が効いたのは、対合が一つだったからだった。]
]
