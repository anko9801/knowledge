...
#link("/math/linear-algebra/5")[前回]の結果を使うと、この事情が消える。
...
#link("/math/linear-algebra/2")[第 1 回]で「跡や行列式は写像の性質」と書いたが、
...
#check[
  @thm:multiplicative の証明で、$Lambda^n V^*$ が#strong[$1$ 次元である]ことは
  どこで効いているか。$2$ 次元だったら、この論法のどこが通らなくなるか。
][
  「$f$ を通すと $omega$ が定数倍になる」という一行目で効いている。

  $f^*omega$ は交代 $n$ 形式なので $Lambda^n V^*$ の元である。そこが $1$ 次元なら、
  $omega$ 自身がその空間を張るので $f^* omega = c thin omega$ と書くしかない。
  この $c$ を $det f$ と呼んでいるのだった。

  $2$ 次元だったら $f^* omega$ は $omega$ の定数倍とは限らず、
  #strong[$f$ に数を一つ割り当てること自体ができない]。行列式は行列の性質ではなく、
  $Lambda^n$ が $1$ 次元だという事実の言い換えである。
]
