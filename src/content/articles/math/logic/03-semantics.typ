...
自然数のことだと思えば真である。どんな数にも、それより大きい数がある。
だが $\{0, 1\}$ の話だと思えば偽になる。$1$ より大きいものが無い。
$<$ を「大きい」の意味に取れば、また逆になる。

#strong[同じ記号列が、真にも偽にもなる。]

言われてみれば当たり前だ、と思うかもしれない。何の話か決めていないのだから、
真偽が決まらないのは当然ではないか、と。

その「当たり前」を、正確に書き下すのが今回である。
そして書き下してみると、当たり前ではなかったことが分かる。
文法が正しいかどうかは、記号列だけを見れば決まる。前回それをやった。
だが真偽はそうではない。#strong[真偽は記号列の性質ではなく、
記号列と、それを読む場所との関係である。]

だから今回やることは決まっている。#strong[「場所」を数学的な対象として書き下し、
記号列との関係を定義する。]前回作った記号列に、ここで初めて中身が入る。

そして関係として定義しておくと、あとで効いてくる。関係の一方——場所の側——を
取り替えられるからである。同じ公理系を満たす別の場所を作れれば、
「この公理からは証明できない」が言えるようになる。
第 6 回の非標準モデルも、集合論の独立性証明も、全部この形をしている。
...
#check[
  @def:satisfaction は $and$ の意味を「かつ」と書き、
  $forall$ の意味を「すべての」と書いている。
  #strong[これは何も定義していないのと同じではないか。]
][
  #strong[循環ではなく、二つの層のあいだの翻訳である。]

  左辺の $and$ は、前回組み立てた#strong[記号列の中の記号]でしかない。
  それ自体に意味は無く、一意可読性で分解できるというだけの対象である。
  右辺の「かつ」は、こちらが数学をするときに使っている言葉の側にある。

  同じ語が両側に見えても、指しているものが違う。定義が言っているのは
  「記号 $and$ を、こちらの『かつ』へ対応させる」であって、
  「かつ」とは何かを説明してはいない。

  そして説明しないことが要点である。対応さえ決まれば、
  #strong[片側だけを取り替えられる]。同じ公理を満たす別の構造を作れば
  「この公理からは証明できない」が言えるようになる。
  意味を無から作っていたら、この取り替えができない。
]
