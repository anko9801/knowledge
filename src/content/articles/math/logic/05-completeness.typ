...
#example[記号そのものを、要素として使う][
  何もない所からモデルを作るので、材料は#strong[記号しかない]。だから記号を要素にする。

  言語に $0$ と後続関数 $S$ があるとする。閉項は
  $ 0, quad S 0, quad S S 0, quad dots $
  で、これがそのまま領域の候補になる。#strong[$S S 0$ という文字列が、要素の名前である。]

  ただし $T$ に $S S 0 = 0$ が入っていれば、この二つは同じ要素にする。
  だから同値類で割る。$T$ が何を等しいと言っているかが、そのまま要素の同一性になる。

  述語も同じで、$P t$ が $T$ に入っているかどうかで $[t] in P^frak(A)$ を決める。
  #strong[$T$ の中身を、そのまま構造として読み直している]だけである。

  何かを作っているように見えて、実は#strong[$T$ を書き写している]。
  無矛盾でありさえすればモデルが湧いて出るのは、そのためだった。
]
