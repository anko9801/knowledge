...
前回の宿題はこうだった。$Gamma ⊨ phi.alt$ ならば $Gamma tack.r phi.alt$ か。

対偶を取って言い換えると、次の形になる。

#strong[無矛盾な理論には、必ずモデルがある。]

言い換えたところで難しさは変わらない。「矛盾が出ない」というのは、
記号列をどう並べても $phi.alt$ と $not phi.alt$ が両方出てこない、という組合せ的な事実である。
そこから、数学的な構造を一つ作らなければならない。

材料はどこにもない。あるのは記号列だけである。
ならば#strong[記号列そのものを材料にする]。これが Henkin の発想である。
