...
= 実数が非可算であることを、可算な世界が証明する

集合論の公理系 $upright("ZFC")$ を書く。その中で「実数全体は非可算である」を証明する
（Cantor の対角線論法）。ここまでは普通の数学である。

ところが $upright("ZFC")$ に矛盾が無いなら、$upright("ZFC")$ を満たす
#strong[可算な]構造が存在する。前回の作り方で出てくる。

その構造の中には、「実数全体」に当たる元 $r$ がある。
そして構造の中では「$r$ は非可算である」が成り立っている。
だが外から数えると、$r$ の元は#strong[可算個しかない]。

同じものが、中から見ると非可算で、外から見ると可算である。

初めて聞いたとき、どこかに間違いがあると思うのが普通だと思う。
対角線論法が壊れたのか、前回の構成に穴があるのか、と。
#strong[どちらも壊れていない。]これは Skolem のパラドックスと呼ばれているが、
名前に反して矛盾ではない。矛盾に見えるのは、
#strong[「非可算」を絶対的な性質だと思っているから]である。

種は、この記事の中盤で割れる。「非可算」は「全単射が存在しない」という主張で、
その「存在」は#strong[構造の中での存在]にすぎない。数え上げる全単射が
構造の外にあれば、中の住人には見えない。前回の作り方が構造を記号列から
組み立てた以上、入っている元は言語で名指しできるものだけだった。

今回はここから、#strong[大きい構造から小さい部分を切り出しても同じ一階の性質を保てる]
という道具まで進む。集合論で $V_alpha$ から可算な部分を切り出し、
Mostowski 崩壊で推移的モデルを作る——独立性証明の入口は、この切り出しである。

まず、使いやすい形にしておきたい。実際に欲しいのはこういう主張である。
...
#check[
  冒頭の $r$ は、中から見ると非可算で、外から見ると可算だった。
  では#strong[対角線論法はどこへ行ったのか。]
  Cantor の証明は $frak(M)$ の中でも通っているはずである。
][
  通っている。そして#strong[通っていることと、外から可算であることは両立する。]

  対角線論法が示すのは「$omega$ から $r$ への全単射は存在しない」である。
  $frak(M)$ の中でこれを証明したので、$frak(M)$ の要素であるような全単射は無い。
  外にある全単射については、何も言っていない。

  証明が嘘をついているのではなく、#strong[「存在しない」の範囲が
  モデルの中に限られている]。定理の中身ではなく、量化子の走る先の話である。
]
