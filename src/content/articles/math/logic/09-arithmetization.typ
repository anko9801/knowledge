...
算術の言語に用意されている記号は四つしかない。$0$、次の数を作る $S$、
$+$、$times$。これで書けるのは数についての主張だけである。
$2 + 2 = 4$ は書ける。$forall x med exists y med (y > x)$ も書ける。

では、こう書けるだろうか。

#strong[「この論理式は $upright("PA")$ で証明できない」]

書けるはずがない、と思うのが自然だと思う。「論理式」は記号の列で、
「証明」はその列の有限列である。どちらも数ではない。
数のことしか言えない言語で、記号列について語れるわけがない。

#strong[語れてしまう。]今回やるのはそれである。

種は拍子抜けするほど単純で、#strong[記号列に番号を振る]。
論理式が数になり、証明が数になれば、
「この数は、あの数の証明を符号化している」は数についての主張になる。
四つの記号でも書ける主張になる。

そして番号を振ったあとに、もっと妙なことが起きる。
#strong[自分自身の番号について語る文]が作れるようになるのだ。
今回の終わりに出てくる対角化補題がそれで、次回の不完全性定理は
そこに一行を代入するだけで出る。
...
#example[実際に番号を振ってみる][
  記号に番号を振る。$0$ に $1$、$S$ に $3$、$=$ に $5$ とする。

  記号列 $S 0 = 0$ は四つ組ではなく三つ組なので、素数を三つ使って
  $ ⟦S 0 = 0⟧ = 2^3 dot 3^1 dot 5^5 = 8 dot 3 dot 3125 = 75000 $
  になる。

  逆に $75000$ を素因数分解すると $2^3 dot 3^1 dot 5^5$ で、
  指数を順に読めば $3, 1, 5$、記号に戻せば $S, 0, =$ ——
  #strong[並び順まで込みで、一意に戻る]。

  戻せることが効いている。「$x$ は論理式の番号か」は、
  $x$ を素因数分解して構文の規則に照らすだけで判定できる。
  #strong[数についての問いが、記号列についての問いと同じものになった。]
]
