...
#check[
  @thm:diagonal の証明を見ると、$sigma = theta(bold(k))$ の中に書かれている数は
  $k$ である。$k$ は $theta$ の Gödel 数であって、$sigma$ の Gödel 数 $d(k,k)$ ではない。
  #strong[$sigma$ は自分の番号を持っていない。どこが自己言及なのか。]
][
  #strong[持てないので、代わりに「自分を作る手続き」を持たせている。]

  自分の番号を直接書き込もうとすると壊れる。番号を書き込めば文が長くなり、
  長くなれば番号が変わり、書き込むべき番号も変わる。#strong[無限に後退する。]
  だからこの道は最初から塞がっている。

  証明がやったのは別のことである。$sigma$ が持っているのは
  $D(bold(k), bold(k), z)$ ——「$k$ 番の論理式に $k$ を代入したものの番号が $z$ である」
  という#strong[計算の記述]である。$z$ が何になるかは、$sigma$ の中には書かれていない。
  読むときに計算されるだけである。

  そして $theta$ に $k$ を代入したものが $sigma$ なのだから、
  その計算結果はちょうど $sigma$ の番号になる。
  #strong[書き込まずに、指すことに成功している。]

  自分のソースコードを文字列として持たないプログラムが、
  自分を出力できるのと同じ仕掛けである。持つのは文字列ではなく、
  文字列を復元する手順のほうになる。
]
