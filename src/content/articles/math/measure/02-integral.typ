...
= 交換できるようになった代わりに、条件収束を捨てた

極限と積分を交換したかった。そのために積分を作り直して、各点収束で交換できるところまで来た。

#strong[払ったものがある。] Lebesgue 可積分とは $integral abs(f) < infinity$ のことで、
絶対値を付けた側が有限でなければ、$f$ 自身がどう振る舞っても可積分にならない。

$sin x \/ x$ を $0$ から $infinity$ まで積分すると、Riemann の広義積分としては
$pi \/ 2$ になる。#strong[Lebesgue 積分としては定義されない。] 正の部分と負の部分が
どちらも無限大で、打ち消し合って有限になる、という値の付き方を許していないからである。

打ち消し合いを許すと、足す順序で答えが変わる。順序に依らない値が欲しくて絶対収束を
要求した結果、順序に依って値が決まる積分が範囲の外に出た。#strong[交換の自由と、
条件収束は、同時には持てない。]

道具はそろった。次回、全測度を $1$ に正規化する。それだけで確率論になる。
可測関数は確率変数と呼ばれ、積分は期待値と呼ばれ、積測度は独立性と呼ばれる。
#strong[名前が変わるだけで、中身はここまでと同じ]である。
