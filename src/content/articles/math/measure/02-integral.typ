...
#check[
  前回の想起。Vitali 集合が示したのは何で、その対策として何をしたか。
][
  「長さ」を$RR$ の#strong[すべての部分集合]に、平行移動不変かつ可算加法的に
  割り当てることはできない、と示した。技術的な不備ではなく避けようがない。

  対策は、測る対象を先に制限すること。可算合併と補集合で閉じる族＝$sigma$ 加法族を
  決めてから、その上で測度を定義する。
]
