...
$[0,1]$ に向かってダーツを投げる。刺さる場所に偏りは無いとする。

ちょうど $0.5$ に刺さる確率はいくつか。$0$ である。
$0.3$ でも $0$、$1\/pi$ でも $0$。#strong[どの一点についても $0$ である。]

それでもダーツはどこかに刺さる。起きたことの確率は $0$ だった。

「確率 $0$ の事象は起こらない」と習った人には、ここで話が止まる。
実際、素朴な定義では止まる。#strong[同様に確からしい場合の数の比]で定義すると、
分母が無限になって比が書けない。#strong[頻度の極限]で定義しても、
何度投げても二度と同じ点には刺さらないので、頻度はいつまでも $0$ のままである。

止まらない道は、前回までに通ってきている。
#strong[「確率 $0$」を「起こらない」と読むのをやめ、「測度が $0$」と読む。]
$[0,1]$ の一点は零集合で、零集合は「空ではないが長さが無い」集合だった。
有理数全体を全部集めても長さが無い、というのと同じことである。

そう読み替えると、奇妙さは残らない。可算加法性は#strong[可算個]の和までしか
主張しないので、非可算個の一点を寄せ集めた $[0,1]$ の測度が $1$ でも矛盾しない。
前回の「可算」の制限が、ここで効いている。

確率論に固有の言葉は一つも要らない。測度論の言葉を借りてきて、
名前を付け替えるだけで確率論になる。

読み替えは、ここで終わらない。同じ手つきを最後まで押していくと、
#strong[$sigma$ 加法族が「どこまで知っているか」を表す]ところまで行き着く。
測る対象を決めるための道具だったものが、情報の粗さの目盛りになる。
ダーツの一点から、そこまでが今回の道のりである。

#check("前回のふりかえり")[
  極限と積分を交換するのに、Riemann は一様収束を要求し、Lebesgue は
...
#link("/math/linear-algebra/8")[線形代数 第 7 回]で見た射影が、
そのまま無限次元で使われている。
...
= 計算の規則は決まったが、意味は決まっていない

道具立ては前回までで終わっていて、今回やったのは翻訳だけだった。
#strong[確率論に固有の公理は無い。]

その代わり、#strong[確率が何を意味するのかも、ここからは出てこない。]

「この硬貨が表になる確率は $1\/2$」と書いたとき、何を主張しているのか。
何回も投げれば半分に近づく、と読むこともできるし、次の一回についての
自分の確信の度合いだ、と読むこともできる。$Omega$ を何に取るかも自由で、
実際この回では#strong[$Omega$ が何かは問題にならない]と書いた。

どちらの読み方も、コルモゴロフの公理を等しく満たす。#strong[満たすので、
数学の側から選べない。] 測度論は計算の規則を与えているだけで、
その規則が何についてのものかは、外から持ってくることになる。

これは欠陥ではない。#link("/math/set-theory/2")[集合論 第 2 回]で
「公理は真理ではなく道具の設計仕様である」と書いたのと同じ形で、
仕様を満たす実装が複数ある、というだけである。ただし#strong[複数あることを
忘れると、片方の読み方でしか正しくない議論を、全体に広げてしまう]。

#link("/math/measure/4")[次回]、@thm:chebyshev と
@prop:independent-var を組み合わせて大数の法則を出す。
そこで出るのは「何回も投げれば近づく」のほうで、
#strong[それが定理として出るということは、意味の側の主張ではない]。
