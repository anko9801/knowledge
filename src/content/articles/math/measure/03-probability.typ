...
#check[
  前回の想起。Riemann 積分ではなく Lebesgue 積分を採る、本当の理由は何だったか。
][
  極限と積分を交換してよい条件が、大きく緩むからである。

  Riemann では#strong[一様収束]が要る。Lebesgue では#strong[各点収束]でよい
  （押さえの可積分関数があれば）。一様収束は $sup$ を制御する強い条件で
  めったに成り立たず、各点収束は弱いので使える場面が桁違いに多い。
]
