...
#link("/math/linear-algebra/5")[線形代数 第 4 回]で見たとおり、
...
#check[
  関数を偏微分しても、こういうことは起きない。$partial_i f$ はちゃんと
  $1$ 形式の成分になる。#strong[関数とベクトル場で、何が違うのか。]
][
  値がどこに住んでいるかが違う。

  関数の値は数で、$RR$ は場所によらず同じ一つの空間である。だから $f(p+h)$ と
  $f(p)$ の引き算に問題が無い。

  ベクトル場の値は $T_p$ の元で、#strong[点ごとに別のベクトル空間]である。
  $X_(p+h) - X_p$ が定義されていない。成分で引き算できるように見えるのは、
  座標基底を通じて勝手に同一視しているからで、その基底が場所によって回っている。

  つまり困難は微分の側ではなく、#strong[比べる相手が別の空間にいる]ことの側にある。
  だから次に用意するのは「うまい微分」ではなく、
  #link("/math/riemannian-geometry/3")[離れた 2 点のベクトルを比べる規則]のほうになる。
]
