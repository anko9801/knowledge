...
物理の教科書がすべてを下付きで書くのは、この操作で型を潰しているからである。
潰していることを忘れなければ問題ない。
...
#remark[
  ...
  ゲージ理論では、接続のほうが主役になる。
  電磁場のベクトルポテンシャル $A_mu$ は、
  時空の接続ではなく#strong[内部空間の接続]である。
  そして電磁場テンソル $F_(mu nu)$ が、その曲率にあたる。
  この対応は#link("/math/riemannian-geometry/5")[第 5 回]で触れる。
]
