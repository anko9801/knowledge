...
#check[
  $partial_j X^i$ はテンソルにならない。@prop:gamma より $Gamma$ もテンソルではない。
  テンソルでないもの二つを足して、テンソルになる。
  #strong[これは偶然か。]
][
  #strong[偶然ではない。$Gamma$ は、その打ち消しのために入れたものである。]

  座標を替えたときに出る余分な項が、両者で同じ形をしている。
  符号が逆なので、足すと落ちる。@prop:gamma の第二項は、
  前回 $partial_j X^i$ に出た項と対になっている。

  だから「$Gamma$ がテンソルでない」は欠陥ではない。
  #strong[テンソルだったら打ち消せない]。座標の都合で動く量を消すには、
  同じだけ座標の都合で動くものを足すしかない。

  同じ理由で、$Gamma$ を「その点での曲がり具合」と読んではいけない。
  一点で $0$ にできる（正規座標）のは、それが対象の量ではないからである。
]
...
この表が、#link("/math/linear-algebra/2")[線形代数 第 1 回]で立てた判定の、幾何版になっている。
