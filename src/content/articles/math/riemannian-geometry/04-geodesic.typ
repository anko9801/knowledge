...
#check[
  ...
  #strong[一点で消せることと、まわりでも消えていることが別だからである。]

  正規座標が消しているのは $Gamma$ の値であって、その微分ではない。
  実際 $g$ の展開では一階の項まで消えて、
  #strong[二階の項に $R$ が残る]。座標をどう選んでも、この項までは落ちない。

  見えるようにするには#strong[二本要る]。一本の測地線だけを見ていると、
  その上ではいつでも「まっすぐ進んでいる」と書けてしまう。
  二本並べると離れ方に差が出て、それが測地線偏差の方程式の右辺になる。

  だから曲率は $Gamma$ の大きさではなく、$Gamma$ を消しても残るもののほうを測っている。
  #link("/math/riemannian-geometry/2")[第 2 回]で
  $Gamma$ がテンソルでなかったのと、表裏の関係にある。
]
