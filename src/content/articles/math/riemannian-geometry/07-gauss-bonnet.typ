...
#check[
  $K$ も $upright("vol")$ も計量に依る。計量を替えれば、どちらも各点で変わる。
  それを掛けて足し上げたものが、@thm:gauss-bonnet では計量を一つも含まない。
  #strong[積分の途中で、計量はどこへ消えたのか。]
][
  #strong[各点では消えていない。足し合わせたときに消える。]

  計量を少し動かすと $K thin upright("vol")$ も動く。ところがその変化分は
  完全微分の形に書けて、境界が無いので積分すると落ちる。
  #strong[各点の値は動くのに、総和だけが動かない]。

  球を凹ませた例が、それを目で見た形である。凹んだところで $K$ が減り、
  まわりで増え、合計は $4 pi$ のまま。減らした分がどこかへ現れるのは、
  現れなければ総和が変わってしまうからである。

  だから左辺は#strong[計量の連続変形について不変な量]になっている。
  連続変形で移り合わない違いだけが残り、それが位相である。
]
...
同じ対象を、仮定するものを変えながら四回書いてきた。

#table(
  columns: (auto, 1fr),
  [#link("/math/linear-algebra/2")[線形代数]], [ベクトル空間だけ。$1$ 点での話],
  [#link("/math/vector-analysis/1")[ベクトル解析]], [$RR^3$。微分構造も計量も混ぜたまま短く書く],
  [#link("/math/differential-forms/1")[微分形式]], [微分構造だけ。計量は使わない],
  [#link("/math/riemannian-geometry/1")[Riemann 幾何]], [計量と接続を足す],
)

学ぶ順に並んでいるわけではない。入口はベクトル解析で、
詰まったところから仮定の少ないほうへ潜り、必要になったら足しに戻る。
そういう読み方を想定して書いた。
