...
#show: post.with(
  ...
  order: 2,
  tags: ("集合論",),
  summary: "関数を「入力から出力を返す規則」と説明しても、規則が何かには答えていない。集合論はそこを諦めて、対を全部集めた袋そのものを関数と呼ぶ。それで全部うまくいく。",
  provides: (
    "relation-order",
    "well-order",
  ),
...
「関数とは何か」と聞かれたら、何と答えるだろうか。
「入力に対して出力を一つ返す規則」だろうか。悪くないが、
#strong[規則とは何か]には答えていない。集合論の言語に、規則などという便利な言葉は無い。

だから集合論は、そこを諦める。#strong[関数とはグラフのことである。]
入力と出力の対を全部集めた集合、それが関数の正体だ、と言い切ってしまう。
$x$ から $y$ を「作る」手続きはどこにも無く、あるのは対の一覧だけである。
...
#check[
  $bb(N)$ と偶数全体のあいだには全単射がある。大きさは同じである。
  切片 $pred(bb(N), a, <)$ を取ると有限集合になるので、これは大きさから違う。
  では#strong[無限に長い切片]を持つ整列順序を持ってくればよさそうに見える。
  #strong[それでも同型にならないと言い切れるのはなぜか。]
][
  #strong[全単射があるかと、順序を保つ全単射があるかが別の問いだからである。]

  大きさのほうは、対応さえ付けばよいので自由度が大きい。
  $bb(N)$ と偶数全体を結ぶ $n mapsto 2n$ は、その自由度を使っている。

  順序を保てと言った途端に @lem:no-decrease が効く。
  整列順序の上では、順序を保つ単射はどの点も自分より下へ落とせない。
  ところが切片への同型は $a$ を $a$ より下へ送ることになる。

  #strong[大きさが同じでも、並びの形が違えば同型にならない。]
  この「並びの形」を測る目盛りが順序数である。

  なお整列性が要る。$bb(Z)$ 上の $n mapsto n - 1$ は順序を保つ単射で、
  全部を下へ落としている。@lem:no-decrease はそこで成り立たない。
]
