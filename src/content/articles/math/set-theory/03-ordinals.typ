...
#show: post.with(
  ...
  order: 3,
  tags: ("集合論",),
  summary: "順序の定義をどこにも書かないまま、大小が決まる。$in$ がそのまま大小になっているからである。代償として、この作り方で全部を集めようとすると体系が壊れる。",
  provides: (
    "ordinal",
...
順序の定義はどこにも書いていない。それでも $2 < 3$ が言える。
$2 in 3$ だからである。#strong[大小が、$in$ そのものになっている。]

目盛りが自分より小さい目盛りを全部抱えているからで、
初めて見たときは、うまくやったものだと唸った。

そして代償がある。この作り方で全部を集めようとすると、体系のほうが壊れる。

なぜこんな形が要るのかから書く。#link("/math/set-theory/2")[第 2 回] の比較定理により、
...
von Neumann の答えが、上の定義だった。同型類を作る代わりに、
各類から代表元を一つずつ選ぶ。しかも自分より小さいものを全部抱えた代表を選ぶ。
