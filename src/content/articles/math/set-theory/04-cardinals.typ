...
#show: post.with(
  ...
  order: 4,
  tags: ("集合論",),
  summary: "平方数は自然数の一部でしかないのに、一対一にきれいに対応がつく。Galileo はここで無限に大小は無いと結論した。捨てるべきだったのは「部分のほうが小さい」という有限での常識のほうだった。",
  provides: (
    "cardinal",
    "cantor-theorem",
  ),
...
= 平方数のほうが少ないはずなのに、数が合う

$1, 2, 3, dots$ と $1, 4, 9, dots$ を並べる。
平方数は自然数の一部でしかないので、明らかに少ないはずである。
それなのに $n arrow.r.bar n^2$ できれいに一対一に対応してしまう。

Galileo はここで困惑して、#strong[無限に大小など考えられない]と結論した。
まともに扱ってはいけない問いだ、というのが長いあいだの答えだった。
