...
#show: post.with(
  ...
  order: 6,
  tags: ("集合論",),
  summary: "$in$ の循環を禁じただけの正則性公理が、実は宇宙の全体像を規定している。作れる順に並べた階層と、そこへ下から届かない基数。そして CH が独立であるとはどういうことか。",
  provides: (
...
正則性公理は、$in$ の循環を禁じる細かい規則にしか見えない。
$x in x$ のような不作法を後から塞ぐだけの、後始末のような一行である。

ところがこれは、#strong[すべての集合が、ある階層のどこかに現れる]という主張と
同値になる。禁止の規則だと思っていたものが、宇宙の全体像を述べていたことになる。

その階層のほうから始める。#link("/math/set-theory/1")[第 1 回] で、公理的集合論は
...
#check[
  $aleph_omega$ は $aleph_0$ より大きく、$aleph_1$ よりも、$aleph_(100)$ よりも大きい。
  それなのに可算個の階段で届いてしまう。
  #strong[大きいのに近い、という言い方は矛盾していないか。]
][
  #strong[大きさと、到達に要する歩数が別の量だからである。]

  濃度が測っているのは、その集合に何個の元があるかである。
  共終数が測っているのは、下から近づくときに#strong[何本の足場が要るか]で、
  一本ずつの足場がどれだけ大きくても構わない。

  $aleph_omega$ は $aleph_0, aleph_1, aleph_2, dots$ という可算個の足場で届く。
  足場そのものはいくらでも大きくなるが、#strong[本数は可算のまま]である。
  だから $cf(aleph_omega) = omega$ になる。

  区別が効くのは @thm:koenig である。あの定理が $kappa$ を縛るのに使うのは
  濃度ではなく共終数のほうで、だから $2^(aleph_0)$ に付く制約も
  「$cf$ が $omega$ より大きい」という形になる。
  #strong[大きさを聞いても答えが出ないところで、歩数が答える。]
]
