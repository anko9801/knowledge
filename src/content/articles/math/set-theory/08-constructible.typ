...
#example[$Def$ と $cal(P)$ は、どこから離れるか][
  $X = \{0, 1, 2\}$ のような有限集合では、二つは一致する。
  $\{0, 2\}$ は $x = 0 or x = 2$ で書けるし、どの部分集合も要素を名指しして書き下せる。
  #strong[有限のうちは、定義できないものが無い。]

  $X = omega$ にすると離れる。$cal(P)(omega)$ は非可算だが、
  論理式は有限の記号列で、パラメータも $omega$ の要素なので、
  $Def(omega)$ は#strong[高々可算]である。

  つまり $Def(omega) subset.neq cal(P)(omega)$ で、
  #strong[名前の付かない部分集合が、圧倒的多数として捨てられる]。

  捨てているのは病的なものだけではない。
  実際に使う集合の多くは書けるので残るが、
  「どれとは言えないが存在はする」で取ってきたものは落ちる。
]
