...
独立性の証明はこれで完結した。第 1 回から通してやってきたことを、並べておく。
...
残りは 2 本ある。#link("/math/set-theory/11")[第 11 回]は補遺で、
第 9 回で骨格だけにした真理補題を証明する。
そして#link("/math/set-theory/12")[第 12 回]では、独立性の手前に何があるのかを見る。
$upright("CH")$ が決まらないと言っても、集合を単純なものに限れば話は決着している。
どこまでが決着していて、どこから公理に依存するのか。それが最終回である。
