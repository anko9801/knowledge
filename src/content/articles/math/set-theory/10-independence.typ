...
これからやることを先に言っておく。#strong[実数を足す。]

すでにある実数直線に、いままで無かった実数を付け加える。それも
$aleph_2$ 本という具体的な本数だけ足して、$upright("CH")$ を破る。

初めて聞くと、何を言っているのか分からないと思う。実数全体は決まったものであって、
足したり減らしたりするものではない。$sqrt(2)$ を「足す」とは何のことか。

ここが強制法のいちばん躓きやすいところなので、先に言葉を正しておく。
足す先は#strong[モデル]である。第 6 回以降ずっと、$upright("ZFC")$ の
「モデル」を相手にしてきた。モデルとは $upright("ZFC")$ の公理を全部満たす一つの世界で、
そこには「実数全体」に当たる集合がある。だが#strong[その世界が
すべての実数を持っているとは限らない]。第 7 回で見たとおり、
可算なモデルですら「実数は非可算である」を満たしてしまうのだった。

だから、外から見れば存在するのに中には無い実数がある。
それをモデルの中へ入れてやると、中の住人にとっては実数が増える。

問題は、入れたあとも $upright("ZFC")$ が成り立っていなければならないことである。
乱暴に足すと壊れる。#strong[何を足せば壊れないか]を管理するのが強制法で、
今回の技術的な中身はほとんどそこに費やされる。

そして最後に、この作業がもたらしたものを見る。$upright("CH")$ は真でも偽でもなく、
#strong[公理からは決まらない]。数学の問いに、そういう答え方があるという発見のほうが、
$upright("CH")$ 自体よりずっと遠くまで効いている。

道具は#link("/math/set-theory/9")[第 9 回]で揃った。作戦はこうなる。
...
#example[条件は、書きかけの表である][
  $I = omega_2 times omega$ の条件 $p$ は、こういう有限の表だと思えばよい。
  ...
  埋まっているのは#strong[有限個の升目だけ]である。残りは未定で、
  $q <= p$ とは「$p$ の埋まっている升目はそのままに、さらに何個か埋めた表」を指す。

  $G$ が生成的だと、どの升目もいつかは埋まる。
  「$⟨alpha, n⟩$ を埋めた表の全体」が稠密だからで、
  #strong[埋め忘れがあれば埋めればよい]だけである。

  だから $f = union.big G$ は表を全部埋めきったものになり、
  各行が実数一本になる。$aleph_2$ 行あるので、実数が $aleph_2$ 本できる。
]
...
#check[
  @def:ccc が言っているのは「反鎖が可算」だけである。
  可算という一語しか出てこない。
  #strong[それでどうして $aleph_(17)$ まで一斉に守れるのか。]
  大きい基数ほど強い条件が要りそうに見える。
][
  #strong[守っているのは基数そのものではなく、「新しい全射が現れない」ことだからである。]

  @thm:ccc-preserves の証明を見ると、$beta$ が壊れるとは
  $alpha < beta$ から $beta$ への全射が $M[G]$ に現れることだった。
  @lem:ccc-cover はその全射の値を、$M$ で計算した#strong[可算個の候補]に閉じ込める。

  すると $beta$ は $alpha$ 個の可算集合の和に収まる。濃度は
  $card(alpha) dot aleph_0$ である。$alpha < beta$ で $beta$ が基数なら、
  この積は $beta$ に届かない。#strong[$beta$ が何であっても同じ計算で届かない。]

  効いているのは絞り込みが#strong[一段だけ]で済んでいることである。
  可算に絞れれば、あとは $card(alpha) dot aleph_0 < beta$ という基数算術が
  すべての $beta$ で同時に働く。ccc の「可算」が候補表の「可算」にそのまま移り、
  そこから先は $beta$ の大きさを見ていない。

  逆に、反鎖が $aleph_1$ 個まで許されると絞り込みが $aleph_1$ 個になり、
  $card(alpha) dot aleph_1$ が $aleph_1$ を超えられなくなる。
  $aleph_1$ が壊れる余地が出る。#strong[条件の「可算」は、
  守りたい最小の基数 $aleph_1$ と対になっている。]
]
