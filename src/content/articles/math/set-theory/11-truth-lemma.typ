...
この定義は#strong[$M$ の外を見ている]。$G$ は $M$ の中に無いからである。

ところが同じ関係が、$M$ の中だけの言葉で書き直せる。そして二つは一致する。
強制法が回るのは、この一致のおかげである。

第 9 回では、そこを骨格だけ述べて先へ進んだ。
...
#proof[
  ...
  $ D_q := \{ r <= q : exists ⟨pi, s⟩ in tau_2 med (r <= s and r ⊩^* tau_1 = pi) \} $
  とおくと、
