...
= $delta Q$ の $delta$ は、何をごまかしているのか

熱力学の第一法則を、こう習う。

$ delta Q = dif U + p thin dif V $

そして「$delta Q$ は#strong[不完全微分]だから $dif Q$ とは書かない」と言われる。
不完全とは何のことか、と聞くと「経路に依るから」と返ってくる。
それはそうなのだが、#strong[では $delta$ という記号は何を意味するのか]は、
たいてい説明されないまま先へ進む。

私はここを長いあいだ、記法の慣習として飲み込んでいた。

飲み込まなくてよい。$delta Q$ は完全に普通の $1$ 形式である。
不完全でも半端でもない。ただ#strong[外微分の像になっていない]だけで、
第 4 回の言葉でいえば「閉形式ではない」。$delta$ はそれを注意する印にすぎない。

しかもこの舞台には、もっと面白いことが起きている。
状態空間 $(T, V)$ には#strong[長さが無い]。$T$ と $V$ は単位が違うので、
$sqrt(Delta T^2 + Delta V^2)$ に意味が無い。角度も内積も定義できない。
つまり grad も div も rot も#strong[一つも書けない]。

それでも微分と積分はできて、第一法則は書ける。
$RR^3$ で組み上げた道具立てのうち、計量が要るものだけが落ちて、
要らないものが残る。#strong[何が計量に寄りかかっていたか]が、
ここで一気に見える。
...
#check[
  この舞台には距離も角度も無い。$star$ が書けないので、grad も div も rot も
  一つも定義できない。#strong[それなのに微分と積分ができるのはなぜか。]
][
  #strong[$dif$ と $integral$ は、はじめから計量に寄りかかっていないからである。]

  $dif$ が要求するのは「関数を $1$ 形式へ、$1$ 形式を $2$ 形式へ」という
  次数の上げ方だけで、長さも直交も出てこない。
  積分のほうも、形式と向き付けた領域を組ませるだけである。

  落ちるのは $star$ を経由するものに限られる。
  $nabla$ 記法の三つは、どれも $1$ 形式とベクトルを行き来させるので、
  #link("/math/vector-analysis/2")[第 2 回]で見たとおり内積が要る。
  内積が無ければ、そもそも書けない。

  つまり $RR^3$ で一体に見えていた道具立ては、
  #strong[計量を要るものと要らないもの]が混ざってできていた。
  熱力学の状態空間は、計量を要らないほうだけがある場所である。
]
...
#link("/math/linear-algebra/5")[線形代数 第 4 回]で対称と反対称を分けたが、
...
この連載では「基底に依らない」と何度も書いた。
その中身は#link("/math/linear-algebra/2")[線形代数 第 2 回]にある。
