...
長さ $ell$ の糸で吊るした振り子を、Newton の流儀で解いてみる。
働く力は重力と糸の張力の二つ。ところが#strong[張力の大きさが分からない]。
糸がどれだけ引くかは、その瞬間の運動によって決まるからである。

だから未知数として置き、糸が伸びないという条件を書き、
連立させて張力を消す——という手順を踏まされる。二重振り子ならもっと増える。

同じ問題を、これから作った形式で解く。

#strong[張力は最初から出てこない。]座標を糸の角度 $theta$ ひとつに取ると、
糸が伸びないことは座標の取り方の中にすでに入っている。
Lagrangian を書いて $theta$ で微分するだけで、答えが出る。
消す作業が要らない。#strong[書かなかったものは、消さなくてよい。]

なぜこんなことが起きるのか。ここまで三回、力という語を一度も使わずに
形式を組み立ててきた。要請から $S = integral L thin dif t$ の形が出て、
停留を課すと Euler--Lagrange 方程式が出て、対称性を課すと $L$ が決まった。
#strong[拘束力は「力」の側の概念だったので、最初から入る場所が無かった。]

とはいえ、都合よく消えたものが本当に不要だったのかは別問題である。
今回はそこを確かめる。この形式が Newton 力学を含むこと、
そして#strong[何を失ったか]まで見る。得したぶんの代金は、最後の節にある。
...
#check[
  @ax:dalembert を「拘束力は仕事をしない」と覚えたくなる。
  だが動くエレベータの床は、乗っている人を押し上げて#strong[仕事をしている]。
  床からの垂直抗力は拘束力である。原理は破れているのか。
][
  #strong[破れていない。原理が言っているのは実際の変位ではなく、仮想変位に対してである。]

  @def:virtual-displacement は時刻を止める。$t$ を固定するので
  $partial bold(r) \/ partial t$ の項が落ち、床は動かないものとして扱われる。
  止めた床の上での仮想変位は床に沿い、垂直抗力とは直交する。仕事は $0$ である。

  実際の変位 $dif bold(r)$ には $partial bold(r) \/ partial t thin dif t$ が入る。
  こちらは床の上昇そのものなので、抗力と平行な成分を持ち、仕事をする。

  #strong[$delta$ と $dif$ を分けて書くのは、まさにこの差のためである。]
  記号を分けるのが面倒に見えるが、分けないと @ax:dalembert が
  成り立たない例がすぐ出てくる。
]
