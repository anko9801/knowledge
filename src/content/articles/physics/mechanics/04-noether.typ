...
運動量が保存する、エネルギーが保存する、角運動量が保存する。
高校でも習うこれらの法則は、それぞれ別の事実として教えられることが多い。

だが解析力学の枠組みでは、三つとも同じ一つの定理の系である。
しかも、その定理は驚くほど短く証明できる。

種明かしを先に言ってしまうと、こうである。

#strong[$L$ が何かの変換で変わらないなら、その変換に対応する保存量がある。]
...
この区別は、次回の Hamilton 形式で $E$ が主役になったときにも効いてくる。
...
$L$ の形そのものを絞り込む。#link("/physics/mechanics/1")[第 1 回]で結論だけ予告した、
