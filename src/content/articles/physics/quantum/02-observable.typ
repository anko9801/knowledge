...
自己随伴作用素まではきれいに出た。測定値が実数で、固有ベクトルが正規直交基底を
なしてほしい、と要求すればスペクトル定理が答える。ここに解釈は入っていない。

だが「係数の絶対値の $2$ 乗が確率である」は、そこから出てこない。
線形代数は $abs(c)^2$ という数を作ってくれるが、
#strong[それを確率と呼んでよい理由は与えていない。] $2$ 乗でなく $4$ 乗でも、
規格化さえすれば足して $1$ にはなる。

$2$ 乗でなければならない理由を、あとから示した定理はある。Gleason の定理が
それで、確率の割り当てに自然な条件を課すと $abs(c)^2$ しか残らない、と言う。
#strong[ただしそれも仮定を置いた上での話]で、しかも $3$ 次元以上でしか成り立たない。

だから公理の一覧を眺めるときは、#strong[どれが翻訳でどれが追加なのかを分けて]
見るのがよい。Hilbert 空間も自己随伴作用素も翻訳だった。
確率の出どころだけが、いまも追加のままである。

次回、系を二つに増やす。そこで初めて、古典では起こりえないことが出る。
