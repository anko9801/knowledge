...
#show: post.with(
  ...
  order: 3,
  tags: ("量子力学",),
  summary: "二つの粒子の状態を余すところなく指定しても、片方だけを測ると結果は完全にでたらめになる。古典では起こりえない。新しい仮定は一つも足しておらず、「系を合わせる」の意味を決めた時点で出てくる。",
  provides: (
    "composite-system",
    "entanglement",
  ),
...
= 全体は決まっているのに、部分が決まらない

二つの粒子について、全体の状態を余すところなく指定したとする。
それでも片方だけを測ると、結果は#strong[完全にでたらめ]になる。

古典では起こりえない。同時分布が決まれば周辺分布も決まり、
同時分布が一点に集中しているなら、周辺分布も一点に集中するからである。

これを出すのに、新しい仮定は一つも要らない。
#strong[「系を合わせる」の意味を一つ決めた時点で、勝手に出てくる。]

その意味のほうから始める。系 $A$ の状態空間が $cal(H)_A$、系 $B$ が $cal(H)_B$ だとして、
合わせた系の状態空間は何か。
...
#check[
  古典確率では、同時分布が決まれば周辺分布も決まる。全体が一点に集中していれば、
  各部分も一点に集中する。量子では全体が純粋なのに、部分が完全にランダムになる。
  #strong[どこで枝分かれしたのか。]
][
  #strong[合わせ方が、直積ではなくテンソル積だったところである。]

  古典で「全体の状態」と言うとき、それは各部分の状態の組でしかない。
  組を決めれば各成分が決まるのは当たり前で、主張ですらない。

  @def:composite はそう置いていない。次元が和ではなく#strong[積]になるので、
  $psi_A times.o psi_B$ の形に書ける元だけでは足りなくなる
  （@prop:not-simple。$dim A + dim B - 1$ と $dim A dot dim B$ の差）。
  余ったぶんが、部分の組では書けない状態である。

  だから「部分が決まらない」は測定の限界ではなく、
  #strong[部分の組で書けない元を、全体として認めた]ことの帰結である。
  新しい仮定は一つも足していない。

  なお情報が消えたわけではない。全体は純粋なままで、
  失われて見えるぶんは相関の側にある。下の $S(rho_A)$ がその量を数える。
]
