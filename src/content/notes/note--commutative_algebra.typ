...
#block[
$0 = 1$ のとき $A$ は唯一の元 $0$ からなる。このとき $A$ は零環 (zero
ring) といい、$0$ で表される。
...
#block[
$f : A arrow.r B$, $g : B arrow.r C$ が環準同型写像ならば合成写像
$g compose f : A arrow.r C$ も環準同型写像である。
...
#block[
環 $A$ の部分集合 $S$ は、加法乗法に関して閉じていて $A$
の単位元を含んでいるとき $A$ の部分環 (subring) であるという。

]
#block[
$frak(a)$ を環 $A$ の部分集合とする。$frak(a)$ が $A$ の加法部分群でかつ
...
#block[
$frak(a)$ を含んでいる $A$ のすべてのイデアル $frak(b)$ の集合と、剰余環
$A \/ frak(a)$ のすべてのイデアル $frak(b)$
の集合との間には、$frak(b) = phi.alt^(- 1) \( frak(b) \)$

]
#block[
環 $A$ の零因子 (zero divisor) とは、「$0$ を割り切る」元 $x$
のことである。すなわち $A$ のある元 $y eq.not 0$ が存在して $x y = 0$
となる元 $x in A$ のことである。 零元と異なる零因子をもたない環を整域
(integral domain) という ($0 eq.not 1$ としている)。
...
#block[
$phi.alt : A arrow.r B$ が環の準同型なら $phi.alt \( 0_A \) = 0_B$
である. $A$, $B$, $C$ を環, $phi.alt : A arrow.r B$, $psi : B arrow.r C$
を準同型とするとき, その合成 $phi.alt compose psi : A arrow.r C$
も準同型である. 同様に $phi.alt$, $psi$ が同型なら,
$phi.alt compose psi$ も同型である.
...
\$\\mathop{\\mathrm{Ker}}{\\phi} = \\Bab{0}\$

]
($arrow.r.double.long$) $phi.alt$ が環の準同型であるから
$phi.alt \( 0_A \) = 0_B$ より $0_A in "Ker" phi.alt$. また元
$forall x \, y in "Ker" phi.alt$ について $phi.alt$ の単射性より
