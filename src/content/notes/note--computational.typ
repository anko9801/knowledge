...
<差分法>
#block[
連続な時間 $t$ に対し、離散化された時間のパラメータは微小時間 $Delta t$
を用いて $t_n = n Delta t$ と書く。同様に空間についても
$x_i = i Delta x$ とする。 時間・空間について連続な関数 $f \( x \, t \)$
に対する離散値を $f_i^(\( n \)) := f \( x_i \, t_n \)$ と書く。
空間について $n$ 次精度とは $cal(O) \( Delta x^n \)$ となること

]
#block[
#strong[定理 1] (1階微分の差分公式). \
...
$f \( x plus.minus Delta x \)$ を 2 次まで Taylor 展開すると
...
$f \( x plus.minus Delta x \)$ を 3 次まで Taylor 展開すると
...
#block[
Taylor展開法 Crank-Nicolson 法は時間と空間について2次精度の陰解法
