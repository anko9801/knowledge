...
<maxwell-方程式>
#block[
電場 $bold(E)$ と磁束密度 $bold(B)$ に対して次のような式が成り立つ。
...
運動量密度 $g_j$ と Maxwell の応力テンソル $T_(i j)$
...
物質の運動量密度 $bold(g)_m \( bold(r) \, t \)$、電磁場の運動量密度
...
任意の関数 $chi \( bold(r) \, t \)$ として次のゲージ変換は不変に保つ。
...
任意の電磁ポテンシャル $\( phi.alt \, bold(A) \)$ に対してゲージ変換し
...
<さまざまな環境>
#block[
#strong[定理 8] (Coulomb 力). \

点電荷 $Q$ を $bold(r)'$ に配置したときに位置 $bold(r)$
での電位と電場、点電荷 $q$ に及ぼす力は次のようになる。
...
点電荷 $+ Q \, - Q$ をそれぞれ
$bold(r)' + bold(d) \/ 2 \, bold(r)' - bold(d) \/ 2$
...
<電磁波の基礎>
#block[
$rho = 0$, $bold(j) = bold(0)$ において $bold(E) \, bold(B)$
は波動方程式を満たす。
$ nabla^2 bold(E) = 1 / c^2 frac(partial^2, partial t^2) bold(E)\
nabla^2 bold(B) = 1 / c^2 frac(partial^2, partial t^2) bold(B) $

]
$nabla times \( nabla times bold(E) \)$ を Maxwell 方程式を用いて 2
通りに計算する。$rho = 0$, $bold(j) = bold(0)$ より示せる。
...
#block[
#strong[定理 18] (導波管内を伝搬する電磁波). \

]
#block[
#strong[定理 19] (円形断面の導波管内を伝搬する電磁波). \

]
#block[
#strong[定理 20] (直方形型の導波管内を伝搬する電磁波). \

]
== 電磁波の回折
<電磁波の回折>
#block[
#strong[定理 21] (Kirchhoff の積分表示). \

]
#block[
#strong[定理 22] (Fresnel-Kirchhoff の回折積分の公式). \

]
#block[
#strong[定理 23] (Fraunhofer 回折). \

]
#block[
#strong[定理 24] (Fresnel 回折). \
...
原点 $O$ に $z$ 軸に平行な電気双極子があり，正電荷 $q$ と負電荷 $- q$ の
$z$ 座標をそれぞれとする。正電荷と負電荷の間に電流 $I_P$ が流れ、$q$
の値が時間と共に変化するとする。
