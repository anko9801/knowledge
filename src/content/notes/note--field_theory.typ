...
#block[
$K$ が標数 $p > 0$ の体, $n$ が正の整数で $q = p^n$ とおくと
$upright(F r o b)_q : K in.rev x mapsto x^q in K$
をフロベニウス準同型という.
...
#block[
$L \/ M$, $M \/ K$ が有限次拡大なら $L \/ K$ も有限次拡大で
$\[ L : K \] = \[ L : M \] \[ M : K \]$ となる.

]
$l = \[ L : M \]$, $m = \[ M : K \]$, ${ x_1 \, dots.h.c \, x_l }$ を
$L$ の $M$ 上の基底, ${ y_1 \, dots.h.c \, y_m }$ を $M$ の $K$
上の基底とする. \
...
を中間体とする. $alpha$ が $K$ 上分離的なら $M$ 上でも分離的である.
