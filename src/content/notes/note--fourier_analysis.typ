...
c_n & := frac(1, 2 pi) integral_(- pi)^pi f \( x \) e^(- i n x) upright(d) x $

]
例

#block[
#strong[定理 1] (Bessel の不等式). \
...
区間 $\[ a \, b \]$ で連続、 $\( a \, b \)$ で微分可能な関数 $f \( x \)$
について $a < c < b$ となる $c$ が存在して次のようになる。
$ frac(f \( b \) - f \( a \), b - a) = f' \( c \) $

]
#block[
$  & integral_(- pi)^pi sin \( m x \) cos \( n x \) upright(d) x = 0\
 & integral_(- pi)^pi cos \( m x \) cos \( n x \) upright(d) x = pi delta_(m \, n)\
 & integral_(- pi)^pi sin \( m x \) sin \( n x \) upright(d) x = pi delta_(m \, n)\
 & integral_(- pi)^pi cos \( n x \) upright(d) x = 2 pi delta_(n \, 0)\
...
a_n & := 1 / pi integral_(- pi)^pi f \( x \) cos \( n x \) upright(d) x\
b_n & := 1 / pi integral_(- pi)^pi f \( x \) sin \( n x \) upright(d) x $
...
a_n & := 1 / L integral_(- L)^pi f \( x \) cos frac(n pi x, L) upright(d) x\
b_n & := 1 / L integral_(- L)^pi f \( x \) sin frac(n pi x, L) upright(d) x $

]
#block[
$ f \( x \) & = sum_(n = - oo)^oo c_n e^(i frac(n pi, L) x)\
c_n & := frac(1, 2 L) integral_(- L)^L f \( x \) e^(- i frac(n pi, L) x) upright(d) x $
...
$ a_0 & = integral_(- 1)^1 x^(2 m) upright(d) x = frac(2, 2 m + 1)\
a_n & = integral_(- 1)^1 x^(2 m) cos \( n pi x \) upright(d) x\
 & = 2 integral_0^1 x^(2 m) cos \( n pi x \) upright(d) x\
...
$ eta \( 2 \) & = pi^2 / 12\
zeta \( 2 \) & = pi^2 / 6 $
$ 1 / L integral_L^(- L) \( f \( x \) \)^2 upright(d) x & = a_0^2 / 2 + sum_(n = 1)^oo \( a_n^2 + b_n^2 \) $
$ zeta \( 4 \) = pi^4 / 90 $ 奇関数 $f \( x \) = x^(2 m - 1)$
$ f \( x \) & = sum_(n = 1)^oo (b_n sin \( n pi x \))\
b_n & = integral_(- 1)^1 x^(2 m - 1) sin \( n pi x \) upright(d) x\
