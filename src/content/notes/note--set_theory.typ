...
#block[
集合に対してある条件 $phi.alt$ を満たしたものを集めた集合がある。
...
#block[
集合 $A$ 関係 $R$ に対し, 次を満たす組 $⟨A \, R⟩$ を狭義全順序と呼ぶ。
...
<順序数>
#block[
集合 $x$ の任意の要素が同時に $x$ の部分集合でもあるとき $x$
が推移的であると呼ぶ。

]
#block[
推移的な集合 $x$ が $in$ によって整列順序づけされるとき, $x$
を順序数と呼ぶ。
