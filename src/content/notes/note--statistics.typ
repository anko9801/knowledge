...
#definition[
あるエネルギー $E$ のときに実現可能な量子状態数を
$W \( E \)$ とおく。また状態の対数を取ったものをエントロピー $S \( E \)$
とする。 $ S \( E \) & = k_B log W \( E \)\
...
#definition[
絶対温度 (absolute temperature) $T$
を次のように定義する。 $ 1 / T & := frac(upright(d) S, upright(d) E) $
]
この温度の定義は理想気体で正当化される。

#theorem("理想気体")[
理想気体、つまり 3
次元箱型ポテンシャル中の独立な区別できない $N$ 個の粒子について
...
#theorem("Maxwell の関係式")[
$C^2$
級の関数において偏微分は交換できるから次の関係式を満たす。
...
#theorem[
独立な調和振動子の角振動数に関する個数分布関数
$g \( omega \)$ について幾何学的解釈で近似できる.
...
]  現実の物質に Debye 模型を当てはめるときには,
それぞれの物質は固有の Debye 角振動数 $omega_D$ を持つことになる.

#theorem[
Debye
模型における内部エネルギーの表式は次のようになる.
...
$V$ は示量変数、$T \, mu$ は示強変数であるから系の大きさを $lambda$
倍すると $ J \( T \, lambda V \, mu \) & = lambda J \( T \, V \, mu \) $
...
#definition[
上の定理において $alpha = 1$
となる粒子をボース粒子またはボゾン (boson) といい、$alpha = - 1$
となる粒子をフェルミ粒子またはフェルミオン (fermion) という。
