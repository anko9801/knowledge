...
#block[
#block[
#strong[定義 1] (複数の同一粒子系における Hilbert 空間). \
...
Hilbert 空間 $cal(H)^(\( 2 \))$ において交換演算子 (exchange operator)
$hat(E)$ を次のように定義する.
$ hat(E) \|psi⟩ \|psi'⟩ & = \|psi'⟩ \|psi⟩ . $
...
まず粒子状態 $\|psi⟩ \|psi'⟩ = \|alpha⟩ \|beta⟩ \, \|beta⟩ \|alpha⟩$
に対して演算子 $hat(E)^(- 1) \, hat(E)^dagger$ を適用する.
...
Hilbert 空間 $cal(H)^(\( 2 \))$ の任意の観測量 $hat(O)$ について 2
...
1 から 2 を示す. 期待値について $\|Psi⟩ arrow.r hat(E) \|Psi⟩$
...
$X$ を集合とするとき $X$ から $X$ への全単射写像 $sigma : X arrow.r X$
を $X$ の置換という. $sigma \, tau$ を置換とするとき, その積 $sigma tau$
...
$n$ 次対称群 $frak(S)_n$ は群である.

]
$sigma \, tau in frak(S)_n$ に対して $sigma tau = sigma compose tau$
が全単射写像であることを示す. まず $sigma tau$ の全射性について $sigma$
の全射性より任意の $c in X$ に対して $sigma \( b \) = c$ となる $b in X$
があり, $tau \( a \) = b$ となる $a in X$ がある. これより任意の $c$
に対して次を満たす $a$ がある.
...
#block[
#block[
#strong[定義 16] (互換, 巡回置換). \

置換 $sigma in frak(S)_n$ に対して $1 lt.eq i < j lt.eq n$ のとき
...
置換 $sigma in frak(S)_n$ においてある元 $i_1 in X$ を選び,
移していくと鳩ノ巣原理より必ず
...
置換 $sigma in frak(S)_n$ の符号 $"sgn" sigma = \( - 1 \)^sigma$
を次のように定義する.
...
#block[
#strong[命題 20] (Q21-7(ii)). \

置換の符号は well-defined である.

]
次のように定義される差積 $Delta \( x_1 \, dots.h \, x_n \)$ を置換
$sigma in frak(S)_n$ 用いて変数の添字を置換することを考える.
...
置換の符号 $"sgn" : frak(S)_n arrow.r bb(Z)^times$ は準同型写像である.
...
$N$ 個の同一の粒子 $X_1 \, dots.h \, X_N$ からなる全体系の Hilbert 空間
$cal(H)^(\( N \))$ において置換 $sigma in frak(S)_N$ を用いて粒子 $X_i$
の状態を粒子 $X_(sigma \( i \))$ の状態に置き換える演算子を置換演算子
$hat(P) \( sigma \)$ とする.

]
]
#block[
#strong[命題 23] (Q21-9, Q21-10(i)(ii)). \
...
置換演算子の行列表示について置換演算子を適用すると粒子 $X_i$
における状態は元々 $X_(sigma^(- 1) \( i \))$ であるから次のようになる.
...
#block[
#strong[補題 26] (Q21-12(i)(ii)). \

任意の互換 $sigma in frak(S)_N$ に対して
$hat(P) \( sigma \) \|Psi⟩ = \|Psi⟩$,
$hat(P) \( sigma \) \|Psi⟩ = - \|Psi⟩$ となることは完全対称,
完全反対称であることと同値である.

]
任意の置換 $sigma in frak(S)_N$ は互換の積で表現できるから互換
...
の内積について互換 $sigma$ の演算子を挿入することで求まる.
...
演算子 $hat(cal(S))^(\( N \)) \, hat(cal(A))^(\( N \))$
に対して置換演算子 $hat(P) \( tau \)$ を適用すると次のようになる.
...
$1 lt.eq mu < nu lt.eq N$ において $\|psi_mu⟩$ と $\|psi_nu⟩$
が線形従属であるならば
$hat(cal(A))^(\( N \)) \|psi_1⟩ dots.h.c \|psi_N⟩ = 0$ となる.

]
任意の $sigma in frak(S)_n$ に対して $tau \( mu \) = sigma \( nu \)$,
$tau \( nu \) = sigma \( mu \)$ であり, その他の元 $1 lt.eq i lt.eq N$
で $tau \( i \) = sigma \( i \)$ となる $tau$ が一意に取れる. $tau$ は
$sigma$ に対して符号が反転し,
...
#block[
#block[
#strong[定義 33] (完全対称, 完全反対称な状態の基底とその粒子数). \
...
#block[
#strong[定理 34] (Q21-19(i), Q21-20(i)(ii)(iii)). \
...
まず $hat(cal(S))^(\( N \)) \|phi.alt_(i_1)⟩ dots.h.c \|phi.alt_(i_N)⟩$
のノルムを計算すると次のようになる.
...
まず $hat(cal(A))^(\( N \)) \|phi.alt_(i_1)⟩ dots.h.c \|phi.alt_(i_N)⟩$
のノルムを計算すると次のようになる.
...
#block[
#block[
#strong[定義 39] (スピン統計定理). \

$N$ 個の同一の Bose 粒子による Hilbert 空間は完全対称な Hilbert 空間
...
<計算練習>
#block[
#strong[例 40] (Q21-25, Q21-26, Q21-27). \
...
#block[
#block[
#strong[定義 41] (Bose, Fermi 粒子系の量子状態の粒子数表示). \
...
消滅演算子 $hat(a)_i$, 生成演算子 $hat(a)_i^dagger$ を状態
$\|dots.h \, n_i \, dots.h⟩ in cal(H)_(B o s e)$ に適用すると
...
次の計算により $hat(a)_i \, hat(a)_i^dagger$ は互いに Hermite
共役であることがわかる.
...
個数演算子 $hat(n)_i$, 全粒子数演算子 $hat(N)$ はそれぞれ生成消滅演算子,
個数演算子に展開でき, Hermite 演算子であることがわかる.
...
Bose 粒子系における消滅演算子 $hat(a)_i$ と生成演算子 $hat(a)_i^dagger$
において次の性質は定義と同値である.
...
#block[
#strong[定理 57]
(Q21-52(i)(ii)(iii)(iv)(v)(vi)(vii)(viii)(ix)(x)(xi)(xii)(xiii)(xiv)(xv)(xvi)(xvii)(xviii)(xix)(xx)).
\
...
Fermi 系の個数演算子 $hat(n)_i$ と全粒子数演算子 $hat(N)$
は次のように展開でき, Hermite 演算子であることがわかる.
...
#block[
#strong[命題 61] (Q21-55(i)). \

真空状態 $\|upright(v a c)⟩$ は粒子が 1 つもない状態
$\|0 \, dots.h \, 0 \, dots.h⟩$ と同じである.
...
適用する順番が大きい順である為, それ以前の粒子数は必ず $0$ となる.
これより次のように計算できる.
...
#block[
#block[
#strong[定義 64] ($n$ 粒子演算子). \

Hilbert 空間 $cal(H)^(\( n \))$ において粒子交換に関して対称な演算子を
$n$ 粒子演算子と呼ぶ. このとき $n$ 粒子演算子 $hat(f)$ を Hilbert 空間
...
$hat(v)^(upright(t o t))$ で表現できる. 外部から磁場 $B$
をかけた多電子原子を考える。原子番号 $Z$
...
置換に関して対称な演算子であるから $mu_1 \, dots.h \, mu_n$
番目の状態の置換に対して不変であり,
...
#block[
#strong[定理 68] (Q21-61(i)(ii)(iii)(iv)(v)(vi)(vii)(viii), Q21-62,
Q21-63(i)(ii)(iii), Q21-64(i)(ii)(iii), Q21-65(i)(ii)(iii),
Q21-66(i)(ii),
Q21-67(i)(ii)(iii)(iv)(v)(vi)(vii)(viii)(ix)(x)(xi)(xii)(xiii)(xiv)(xv)(xvi),
Q21-68, Q21-69(i)(ii)(iii)(iv),
Q21-70(i)(ii)(iii)(iv)(v)(vi)(vii)(viii), Q21-71(i)(ii)(iii),
Q21-72(i)(ii)(iii), Q21-73(i)(ii)(iii), Q21-74(i)(ii), \
Q21-75(i)(ii)(iii)(iv)(v)(vi)(vii)(viii)(ix)(x)(xi)(xii)(xiii)(xiv)(xv)(xvi),
Q21-76, Q21-77(i)(ii)(iii)(iv)). \
...
#block[
#strong[定理 72] (Q21-81(i)(ii), Q21-82). \
...
全粒指数演算子 $hat(N)$ は粒子数密度演算子
$hat(rho) \( bold(r) \, s_z \)$ を用いて次のように表現できる.
...
全粒子数演算子は生成消滅演算子 $hat(b)_i^dagger \, hat(b)_i$
...
ある粒子 $mu$ の位置座標 $bold(r)_mu in bb(R)^3$ とスピンの $z$ 成分
...
Schrödinger 方程式の両辺に対して左から $\( 1 \, dots.h \, N \|$
を掛けるとそれぞれ次のように計算できる.
...
#block[
#block[
#strong[定義 89] (Heisenberg 表示での場の演算子). \
...
#block[
#strong[定理 90] (Q21-95(iii)(iv)(v)(vi)). \
...
#block[
#strong[定理 92] (Q21-97(ii)(iii)(iv)). \

粒子数密度 $rho \( bold(r) \, t \)$ の保存則が成り立つ.
つまり次の連続の方程式が成り立つ.
...
#block[
#strong[定理 99] (Q21-101(i)(ii)). \

Schrödinger の場 $phi.alt \( bold(r) \, s_z \; t \)$ の古典系の
Hamiltonian に対する正準量子化をすると次のようになる.
...
#block[
#block[
#strong[定義 101] (Q21-102(ii)(iv)). \

Schrödinger 方程式を満たす Schrödinger の場
$phi.alt \( bold(r) \, s_z \; t \)$ について時間成分で Fourier
展開すると次のようになる.
