...
export const concepts: readonly Concept[] = [
  ...
  // 全部持っていく。上に載っているだけで、下を置き換えてはいない。物理でいえば
  // エネルギーと同じ立場で、全部をジュールで測れることと、エネルギー保存から
  // Maxwell 方程式が出ることは、別である。
  //
  // では外から与えるものは何本か。数えると、四つではなく三つだった。
  //
  //   物理  速い記憶は遠くなる
  //   認知  同時に頭へ置ける量は決まっていて、人は間違える
  //   経済  変更は来るし、遅く直すほど高くつく
  //
  // 論理（Rice、停止問題）はここに入らない。定理だからである。数学を仮定に
  // 含めた時点で出てくるので、外から与える必要がない。並べて数えたのが誤り
  // だった。ただし効かないわけではなく、働き方が違う。下の二つに分ける。
  c('expected-change-cost', '手間の総量', '読む時間も、逃した欠陥の修正も、計算資源も、最後は同じ物差しで測れる', 'viewpoint', 'cs', ['optionality', 'feedback-delay']),
  c('irreducible-inputs', '外から与えるもの', '物理・認知・経済の三つ。どれも他の二つからは出てこない', 'viewpoint', 'cs', ['expected-change-cost']),
  c('feasibility-vs-cost', '高くつくとできないは別', '物理と認知と経済は釣り合いの問題、論理は可否の問題。混ぜると判断を誤る', 'viewpoint', 'cs', ['irreducible-inputs', 'soundness-completeness-tradeoff']),
