...
export const concepts: readonly Concept[] = [
  ...
  // 物理でいえばエネルギーと同じ立場で、全部をジュールで測れることと、
  // エネルギー保存から Maxwell 方程式が出ることは、別である。
  //
  // では既約な入力は何本か。数えると、四つではなく三つだった。
  //
  //   物理  光速、Landauer 限界。速い記憶は物理的に遠くなる
  //   認知  作業記憶 4±1、誤り率
  //   経済  割引率、変更頻度
  //
  // 論理（Rice、停止問題）はここに入らない。定理だからである。数学を仮定に
  // 含めた時点で導出されるので、外から与える必要がない。並べて数えたのが
  ...
  c('irreducible-inputs', '既約な外部入力', '物理・認知・経済の三つ。それぞれ固有の定数を持ち、他の組み合わせから出てこない', 'viewpoint', 'cs', ['expected-change-cost']),
  c('feasibility-vs-cost', '不可能とコストは別', '論理は実行可能領域を切り、他の三つは目的関数に入る。境界と勾配の違い', 'viewpoint', 'cs', ['irreducible-inputs', 'soundness-completeness-tradeoff']),
  c('binding-constraint', '効いている制約', '制約が 1 本しか効かない場所に設計判断は無い。判断は衝突点にだけある', 'viewpoint', 'cs', ['feasibility-vs-cost', 'load-tradeoff']),
