...
export const algorithms: readonly Concept[] = [
  ...
  c('majorization', '優越', '並べ替えた部分和が全部以上。**全順序ではなく半順序で、比べられない対がある**', 'definition', 'math', ['convex-set', 'relation-order']),
  c('hardy-littlewood-polya', 'Hardy–Littlewood–Pólya の定理', '優越すること、二重確率行列で写ること、有限回の移転で移れることの三つが同値。**「平等にする操作」が二つの別の言葉で書ける**', 'theorem', 'math', ['majorization']),
  c('schur-convexity', 'Schur 凸性', '優越を保つ関数。**エントロピーもジニ係数もこの族に入る**が、族の中でどれを選ぶかは定理が決めない', 'definition', 'math', ['hardy-littlewood-polya']),
  c('one-number-cannot-decide', '単調量ひとつでは、変換できるかが決まらない', '**エントロピーが増えるのに移れない対がある**（$(2\\/3, 1\\/6, 1\\/6)$ から $(1\\/2, 1\\/2, 0)$）。しかも別の単調量に取り替えても直らない', 'theorem', 'math', ['schur-convexity']),
