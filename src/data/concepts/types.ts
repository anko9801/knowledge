...
/**
 * 狭い設定で成り立っていたことが、広い設定では偽になる関係。
 *
 * `Derivation` の裏返しではない。あちらは「仮定が定理になる」で、こちらは
 * **「定理が偽になる」**。向きも内容も違う。
 *
 * 2026-09-07 に、三つの調査が**互いに知らないまま同じ形に当たった**ので足した。
 * 常微分方程式で一意だった解が偏微分方程式では一意でなくなる、有限次元で
 * 取れた固有基底が無限次元では取れない、量子力学で可逆だった時間発展が
 * 熱方程式では戻せない。どれも `requires` でも `Derivation` でも書けない。
 *
 * **読者がいちばん躓くのがここである。** 前の連載で定理として習ったものが、
 * 次の連載では成り立たない。黙って進むと、読者は自分の理解を疑う。
 */
export type Limit = {
  /** 狭い設定では成り立っている主張 */
