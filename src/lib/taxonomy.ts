...
export const series: readonly Series[] = [
  ...
  },
  {
    ...
    title: '型と計算',
    blurb:
      'D と D→D が同じ大きさになれない、という一つの障害から始める。自己適用を書けてしまうと意味が付かないので、型を付けて避けるか、数える写像を減らして避けるかになった。型の側から強正規化も Curry--Howard もパラメトリシティも出る。そのあと、入出力が置き換えを壊すところから、作用を値にする話へ。全 3 回。',
  },
  {
    slug: 'programs',
    field: 'cs',
    title: 'プログラムの構成',
    blurb:
      '作法を数え上げても終わらないので、逆から行く。動かせない前提は機械・人の保持・人の誤り・変更の四つしかない。それを先に置いて、goto をやめる理由も型を付ける理由も依存の向きも、そこから導く。導けなかったものは作法ではなく好みだと書く。導出が 1 回、実測との照合が 1 回。全 2 回。',
  },
  {
    slug: 'information',
    field: 'cs',
  ...
  },
  {
    ...
    title: 'Linux カーネル',
    blurb:
      'カーネルという名のプロセスは無い。入口は三つしかなく、そこから「何をカーネルに置くか」も「境界をいくつ置くか」も決まる。読む側が鍵を取らない同期、写像の不在を通知として使うメモリ管理、そして 4400 万行を実際に読む手順まで。ここだけは概念ではなく事例を扱うので、版に依るものと依らないものを毎回分けて書く。全 4 回。',
  },
  {
    slug: 'crypto',
    field: 'cs',
