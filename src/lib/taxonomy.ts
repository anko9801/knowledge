...
export const series: readonly Series[] = [
  ...
  },
  {
    ...
    title: 'オートマトンと形式言語',
    blurb:
      '計算量理論の前段。許す記憶の量を絞ると、認識できる言語の族が決まる。状態が有限なら括弧の対応は数えられず、スタックを一本足すと数えられて、三つ揃えることはできない。どれも証明が構成的で、非決定性が力を増やさないことも定理として出る。全 5 回。',
  },
  {
    slug: 'complexity',
    field: 'cs',
  ...
  },
  {
    ...
    title: 'アルゴリズムと下界',
    blurb:
      '速い手続きを並べても終わらないので、逆から行く。計算のモデルを固定すると限界が決まり、モデルを変えると限界が動く。比較しか許さないなら n log n より速くならず、鍵の構造を使えば線形になる。貪欲が最適になるのはマトロイドのとき、そのときだけ。双対を取ると下界が出る。全 7 回。',
  },
  {
    slug: 'learning',
    field: 'cs',
    title: '学習理論',
    blurb:
      'データを増やせば当たる、は経験則に見えるが、当たるための条件は定理になる。仮説の豊かさは個数ではなく場合分けの能力で測り、集中不等式がそのまま証明になる。標本が足りても、その仮説を見つける手続きが無いことは残る。測度と確率が前提。全 4 回。',
  },
  {
    slug: 'relational',
    field: 'cs',
    title: '関係理論',
    blurb:
      'どの製品かは事例だが、この問い合わせが書けるかは論理で決まる。関係代数と一階述語論理は同じ表現力を持ち、正規形も従属性もそこから出る。推移閉包が書けないことも定理である。取引の正しさを直列化可能性で定義するところまで。全 4 回。',
  },
  {
    slug: 'types',
    field: 'cs',
  ...
  },
  {
    ...
    title: '形式手法',
    blurb:
      'Hoare 論理は一本ずつ人が証明する形だが、こちらは機械にやらせる側。状態が有限なら全部試せるので、有限にならないものをどう有限へ落とすかが本体になる。時相論理、モデル検査、抽象解釈、分離論理。停止する検査器は必ず近似になるので、どの近似を採るかの分類でもある。全 5 回。',
  },
  {
    slug: 'information',
    field: 'cs',
